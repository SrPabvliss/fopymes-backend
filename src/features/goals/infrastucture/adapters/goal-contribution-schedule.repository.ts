...
export class PgGoalContributionScheduleRepository implements IGoalContributionScheduleRepository {
  ...
  public static getInstance(): PgGoalContributionScheduleRepository {
    if (!PgGoalContributionScheduleRepository.instance) {
      PgGoalContributionScheduleRepository.instance = new PgGoalContributionScheduleRepository();
    }
  ...
  async findByStatus(status: string, userId?: number): Promise<IGoalContributionSchedule[]> {
    let query = this.db
      .select()
      .from(goal_contribution_schedule)
      .where(
  ...
  async create(scheduleData: Omit<IGoalContributionSchedule, "id">): Promise<IGoalContributionSchedule> {
    const result = await this.db
  ...
  async update(id: number, data: Partial<IGoalContributionSchedule>): Promise<IGoalContributionSchedule> {
  ...
  async markAsCompleted(id: number, contributionId: number): Promise<IGoalContributionSchedule> {
    const result = await this.db
      .update(goal_contribution_schedule)
      .set({
        status: "completed",
